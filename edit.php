...
// 1. 建立连接
$conn = mysqli_connect('localhost', 'root', 'zw', 'test');
...
if (!$connect) {
  ...
  $query1=mysqli_query($connect,"update users set id={$id},name='{$name}',gender={$gender},birthday='{$birthday}',avatar='{$avatar}' where id ={$id};");
  if (!$query1) {
  exit('<h1>查询数据失败</h1>');
}
